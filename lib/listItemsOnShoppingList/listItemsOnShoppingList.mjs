...
const getItems = (shopperID, shoppingListID) => {
    return new Promise((resolve, reject) => {
        console.log("Fetching items with shopperID:", shopperID, "and shoppingListID:", shoppingListID); // Debugging log
        const sql = `
            ...
            WHERE shoppingListID = ? AND shopperID = ? AND ShoppingListItems.isDeleted = 0
        `;
        pool.query(sql, [shoppingListID, shopperID], (error, results) => {
