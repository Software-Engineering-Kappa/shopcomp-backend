import mysql from "mysql2";
import { corsHeaders } from "./cors.mjs";

// Add chains
let addChain = (connection, requestBody) => {
    return new Promise((resolve, reject) => {
        
        // the values to set the new chain to
        const chainName = requestBody.name;
        const chainUrl = requestBody.url;

        // Inserts ONE chain into the Chains table
        const sqlQuery = "INSERT INTO Chains (name, url) VALUES (?, ?);";
        connection.query(sqlQuery, [chainName, chainUrl], (error, results) => {
            if (error) reject(error);
            else resolve({
                id: results.insertId,
                name: requestBody.name,
                url: requestBody.url
            });
        });
    });
};

export const handler = async (event) => {
    const headers = {
        ...corsHeaders(event),
        "Content-Type": "applications/json"
    }

    // specify credentials
    var connection = mysql.createConnection({
        host: process.env.DB_HOST, // DB endpoint url
        user: process.env.DB_USER, // DB root username
        password: process.env.DB_PASSWORD, // DB password for root user
        database: process.env.DB_NAME // DB identifier
    });

    // return JSON object
    let statusCode;
    let body;
    try {
        const requestBody = JSON.parse(event.body);
        
        if (!requestBody.name) {
            // missing name
            statusCode = 400;
            body = { error: "Missing name in request body" };
        } else if (requestBody.name === "") {
            // name empty
            statusCode = 400;
            body = { error: "Name is empty" };
        } else if (!requestBody.url) {
            // missing url
            statusCode = 400;
            body = { error: "Missing url in request body" };
        } else if (requestBody.url === "") {
            // url empty
            statusCode = 400;
            body = { error: "Url is empty" };
        } else {
            const chainAdded = await addChain(connection, requestBody);
            console.log("Chain added: ", chainAdded);

            // success
            statusCode = 200;
            body = {
                id: chainAdded.id,
                name: chainAdded.name,
                url: chainAdded.url
            };
        }
    } catch (error) {
        console.error("Database query error: ", error)
        return {
            statusCode: 400,
            headers: headers,
            body: JSON.stringify({
                error: "Database error",
            })
        }
    } finally {
        connection.end();
    }

    const response = {
        statusCode,
        headers: headers,
        body: JSON.stringify(body)
    };

    return response;
}
