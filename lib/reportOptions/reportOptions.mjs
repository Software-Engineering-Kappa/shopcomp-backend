...
export const handler = async (event) => {
  ...
  try {
    ...
    `
    const [itemNamesResult] = await connection.execute(getItemNames, [listId])
    ...
    `
    const [items] = await connection.execute(getItems, [itemNames, storeIds])
    ...
    `
    const [predictedTotalCosts] = await connection.execute(getPredictedTotalCosts, [itemNames, storeIds])
    console.log("Got estimated costs")
    console.log("predictedTotalCosts: ", JSON.stringify(predictedTotalCosts))
