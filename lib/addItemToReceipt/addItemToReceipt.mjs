...
export const handler = async (event) => {
  ...
  try {
    ...
    if (existingItems.length === 0) {
      ...
      INSERT INTO Items (name, category, storeID, mostRecentPurchaseDate) VALUES (?, ?, ?, ?)
      `
      commandNumber = 3
      const [createItemResult] = await connection.execute(
        createItemCommand, [itemName, category, storeId, date] // mostRecentPurchaseDate is set to the date of the first purchase
      )
    ...
    // Update the Item's mostRecentPrice
    console.log("Updating mostRecentPrice of item")
    const updateItemPriceCommand = `
    UPDATE Items
    SET mostRecentPrice=?, mostRecentPurchaseDate=?
    WHERE ID=? AND mostRecentPurchaseDate<=?
    `
    commandNumber = 5
    await connection.execute(updateItemPriceCommand, [price, date, itemId, date]) // only update if the new purchase date is more recent
    console.log("Updating mostRecentPrice successful")
